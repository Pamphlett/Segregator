...
void teaser::TLSScaleSolver::distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
                                                         const Eigen::Matrix<double, 3, Eigen::Dynamic> &tgt,
                                                         const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_covs,
                                                         const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_covs,
                                                         double*scale,
                                                         Eigen::Matrix<bool, 1, Eigen::Dynamic> *inliers) {}

void teaser::ScaleInliersSelector::distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
                                                               const Eigen::Matrix<double, 3, Eigen::Dynamic> &tgt,
                                                               const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_covs,
                                                               const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_covs,
                                                               double*scale,
    ...
    *scale = 1;
    ...
    assert(src_covs.size() == tgt_covs.size());
    assert(src.cols() == tgt_covs.size());

    int N = src.cols();

    double                   beta = 2 * noise_bound_ * sqrt(cbar2_);
    // Eigen::MatrixXd noise_squared = Eigen::MatrixXd::Constant(1, N, beta);
    double noise_distribution = distribution_noise_bound_;

    Eigen::Matrix<double, 1, Eigen::Dynamic> v1_dist =
                                                     src.array().square().colwise().sum().array().sqrt();
    Eigen::Matrix<double, 1, Eigen::Dynamic> v2_dist =
                                                     tgt.array().square().colwise().sum().array().sqrt();
    ...
    *inliers = norm_inliers;//.cwiseProduct(kl_inliers);
    ...
    std::cout << "Amount of Inliers:   " << inlier_count << std::endl;
}
...
teaser::RobustRegistrationSolver::computeTIMsWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v,
                                                     const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &cov,
                                                     std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &covtiled,
    ...
    // covtiled.reserve(N * (N - 1) / 2);
    covtiled.resize(N * (N - 1) / 2);

    // std::cout<<"\033[1;32m"<< "Debug 1-3"<<"\033[0m"<<std::endl;

#pragma omp parallel for default(none) shared(N, v, vtilde, map, cov, covtiled)
// #pragma omp parallel for shared(N, v, vtilde, map, cov, covtiled)
    for (size_t i = 0; i < N - 1; i++) {
        ...
        // concatenate to the end of the tilde vector (centers)
        vtilde.middleCols(segment_start_idx, segment_cols) = temp.rightCols(segment_cols);
        
        // calculate cov mats
        Eigen::Matrix3d                                                               cm = cov[i];
        std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> temp_cov = cov;
        
        for (size_t k = 0; k < N; ++k) {
            temp_cov[k] += cm;
        }

        // concatenate to the end of the tilde vector (cov mats)
        // std::copy(temp_cov.rbegin() - segment_cols, temp_cov.rbegin(), covtiled.begin() + segment_start_idx);
        std::copy(temp_cov.begin() + i + 1, temp_cov.end(), covtiled.begin() + segment_start_idx);
        
        // std::cout << "tempcov size:  " << temp_cov.size() << std::endl;
        // std::cout << "covtile size:  " << covtiled.size() << std::endl;
    ...
    // std::cout << "vtile size:  " << vtilde.cols() << std::endl;
    return vtilde;
}
...
teaser::RobustRegistrationSolver::solve(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
                                        const Eigen::Matrix<double, 3, Eigen::Dynamic> &dst, 
                                        const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_covariances,
                                        const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_covariances) {
    //
    ...
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> src_covtiled;
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> tgt_covtiled;
    src_tims_ = computeTIMsWithCov(src, src_covariances, src_covtiled, &src_tims_map_);
    dst_tims_ = computeTIMsWithCov(dst, tgt_covariances, tgt_covtiled, &dst_tims_map_);
    end = clock();
    ...
    solveForScaleWithCov(src_tims_, dst_tims_, src_covtiled, tgt_covtiled);
...
void teaser::RobustRegistrationSolver::solveForScaleWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v1,
                                                            const Eigen::Matrix<double, 3, Eigen::Dynamic> &v2, 
                                                            const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_covtiled, 
                                                            const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_covtiled) {
    scale_inliers_mask_.resize(1, v1.cols());
    scale_solver_->distributionInlierSelection(v1, v2, src_covtiled, tgt_covtiled, &(solution_.scale), &scale_inliers_mask_);
    // scale_solver_->solveForScale(v1, v2, &(solution_.scale), &scale_inliers_mask_);
    std::cout << "Scale: " << solution_.scale << std::endl;
}
