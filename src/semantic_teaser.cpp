    ...
    for (int indx = 0; indx < class_num; ++indx) {
        ...
        for (int src_index = 0; src_index < src_mat.rows(); src_index++) {
            for (int tgt_index = 0; tgt_index < tgt_mat.rows(); tgt_index++ ) {
                this->initial_correspondences.points.first.col(index) << src_mat(src_index, 0), src_mat(src_index, 1), src_mat(src_index, 2);
                this->initial_correspondences.points.second.col(index) << tgt_mat(tgt_index, 0), tgt_mat(tgt_index, 1), tgt_mat(tgt_index, 2);
...
void semanticTeaser::solve_for_multiclass_with_cov(const std::vector<pcl::PointCloud<pcl::PointXYZ>> &src_cloud_vec, 
                                                   ...
                                                   const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_matched_covariances,
                                                   const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_matched_covariances) {
    int total_corres_size = 0;
    int src_node_size     = 0;
    int tgt_node_size     = 0;
    for (int i = 0; i < src_cloud_vec.size(); i++) {
    ...
    for (int matched_index = 0; matched_index < src_matched_mat.rows(); matched_index++) {
        this->initial_correspondences.points.first.col(index) << src_matched_mat(matched_index, 0), src_matched_mat(matched_index, 1), src_matched_mat(matched_index, 2);
        this->initial_correspondences.points.second.col(index) << tgt_matched_mat(matched_index, 0), tgt_matched_mat(matched_index, 1), tgt_matched_mat(matched_index, 2);
...
void semanticTeaser::culculate_correspondence(const Eigen::MatrixX3d& src, 
    ...
    for (int src_index = 0; src_index < src.rows(); src_index++) {
        for (int tgt_index = 0; tgt_index < tgt.rows(); tgt_index++ ) {
            this->initial_correspondences.points.first.col(index) << src(src_index, 0), src(src_index, 1), src(src_index, 2);
            this->initial_correspondences.points.second.col(index) << tgt(tgt_index, 0), tgt(tgt_index, 1), tgt(tgt_index, 2);
            this->initial_correspondences.indices.first.push_back(src_index);
            this->initial_correspondences.indices.second.push_back(tgt_index);
            index++;
        }
