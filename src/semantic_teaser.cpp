...
void semanticTeaser::solve_for_multiclass(
  ...
  for (int i = 0; i < src_cloud_vec.size(); i++) {
    ...
    total_corres_size = total_corres_size + src_cloud_vec[i].points.size() *
                                                tgt_cloud_vec[i].points.size();
  }
...
void semanticTeaser::solve_for_multiclass_with_cov(
  ...
  for (int matched_index = 0; matched_index < src_matched_mat.rows();
    ...
    this->initial_correspondences.points.first.col(index)
        ...
        src_matched_mat(matched_index, 1), src_matched_mat(matched_index, 2);
    this->initial_correspondences.points.second.col(index)
        << tgt_matched_mat(matched_index, 0),
        tgt_matched_mat(matched_index, 1), tgt_matched_mat(matched_index, 2);
...
void semanticTeaser::culculate_correspondence(const Eigen::MatrixX3d &src,
  ...
  this->initial_correspondences.points.first.conservativeResize(Eigen::NoChange,
                                                                temp_size);
...
void semanticTeaser::solve_for_bg_points(
    const pcl::PointCloud<pcl::PointXYZ> &src,
    const pcl::PointCloud<pcl::PointXYZ> &tgt) {

  Eigen::MatrixX3d src_matched_mat;
  Eigen::MatrixX3d tgt_matched_mat;
  pcl2eigen(src, src_matched_mat);
  pcl2eigen(tgt, tgt_matched_mat);

  this->initial_correspondences.points.first = src_matched_mat.transpose();
  this->initial_correspondences.points.second = tgt_matched_mat.transpose();
