...
public:
  ...
  /**
   * Perform inlier selection based on scale and point distribution. (with cov mats)
   * @param src [in] a vector of points
   * @param tgt [in] a vector of points
   * @param src_covs [in] vector of src covs
   * @param tgt_covs [in] vector of tgt covs
   * @param inliers [out] a row vector of booleans indicating whether a measurement is an inlier
   */
  virtual void distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
...
public:
   ...
   * @param inliers [out] a row vector of booleans indicating whether a measurement is an inlier
   */
  void distributionInlierSelection(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
                                   const Eigen::Matrix<double, 3, Eigen::Dynamic> &tgt,
                                   const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &src_covs,
                                   const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> &tgt_covs,
                                   double*scale,
...
public:
  ...
  explicit ScaleInliersSelector(double noise_bound, double cbar2, double distribution_bound)
      : noise_bound_(noise_bound), cbar2_(cbar2), distribution_noise_bound_(distribution_bound){};
  /**
...
private:
  double noise_bound_;
  double distribution_noise_bound_;
  double cbar2_; // maximal allowed residual^2 to noise bound^2 ratio
};
...
public:
  ...
  struct Params {
    ...
    /**
     * A bound to conduct compatibility check between distributions.
     */
    double distribution_noise_bound = 1.0;
  ...
  /**
   * Given a 3-by-N matrix representing points with covariances, return distribution-based Translation Invariant Measurements (TIMs)
   * @param v a 3-by-N matrix
   * @param cov a vector containing each point's covariance matrix 
   * @param covtiled each TIMs' covariance matrix
   * @return a 3-by-(N-1)*N matrix representing TIMs (centers)
   */
  Eigen::Matrix<double, 3, Eigen::Dynamic>
  computeTIMsWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v,
  ...
  /**
   * Solve for scale, translation and rotation with covariance pruning. Assumes v2 is v1 after transformation.
   * @param v1
   * @param v2
   * @param src_covariances covariance matrix vector of v1
   * @param tgt_covariances covariance matrix vector of v2
   */
  ...
  void solveForScaleWithCov(const Eigen::Matrix<double, 3, Eigen::Dynamic> &v1,
                            const Eigen::Matrix<double, 3, Eigen::Dynamic> &v2, 
  ...
  void reset(const Params& params) {
    ...
    if (params_.estimate_scaling) {         // quatro_examples의 setparams 에서 false로 해줌 => ScaleInliersSelector를 가리키는 스마트 포인터  scale_solver_ -> (ScaleInliersSelector)
      ...
      setScaleEstimator(
          std::make_unique<teaser::ScaleInliersSelector>(params_.noise_bound, params_.cbar2, params_.distribution_noise_bound));
    }
