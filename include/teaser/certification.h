...
public:
   ...
   * @param rotation_solution [in] a solution to the rotatoin registration
   * problem
   ...
   */
  virtual CertificationResult
  certify(const Eigen::Matrix3d &rotation_solution,
 ...
 *
 * Please refer to:
 * [1] H. Yang, J. Shi, and L. Carlone, “TEASER: Fast and Certifiable Point
 * Cloud Registration,” arXiv:2001.07715 [cs, math], Jan. 2020.
 */
...
public:
  ...
  struct Params {
    ...
    /**
     * Square of the ratio between acceptable noise and noise bound. Usually
     * set to 1.
     */
    double cbar2 = 1;
   ...
   * @param cbar2 [in] maximal allowed residual^2 to noise bound^2 ratio,
   * usually set to 1
   */
   ...
   */
  CertificationResult
  certify(const Eigen::Matrix3d &R_solution,
          const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
          const Eigen::Matrix<double, 3, Eigen::Dynamic> &dst,
   ...
   * @param src [in] vectors under rotation
   * @param dst [in] vectors after rotation
   * @param theta [in] binary (1 vs. -1) vector indicating inliers vs.
   * outliers
   * @return  relative sub-optimality gap
   */
  CertificationResult
   ...
   * Given an arbitrary matrix W, project W to the correct dual structure
    ...
    (3) W_dual must also satisfy complementary slackness (because M_init
   satisfies complementary slackness) This projection is optimal in the sense
   of minimum Frobenious norm
   */
  void getOptimalDualProjection(
   ...
   * 2. diagonal blocks of (Q - Lambda_guess) is PSD (except the first
   * diagonal block)
   *
   * @param R [in] rotation matrix
   * @param theta [in] a binary (1 & -1) vector indicating inliers vs.
   * outliers
   * @param src [in]
   * @param dst [in]
  ...
  /**
   * Calculate the inverse of the linear projection matrix A mentioned in
   * Theorem 35 of our TEASER paper [1].
   *
   * @param theta_prepended [in] a binary (1 & -1) vector indicating inliers
   * vs. outliers, with 1 prepended
   * @param A_inv [out] inverse of A
   */
  void getLinearProjection(
      const Eigen::Matrix<double, 1, Eigen::Dynamic> &theta_prepended,
      SparseMatrix *A_inv);
