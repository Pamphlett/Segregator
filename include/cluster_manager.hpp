 ...
 *
 * DCVC Code based on: T-LOAM: Truncated Least Squares LiDAR-Only Odometry and
 *Mapping in Real Time link: https://github.com/zpw6106/tloam
 *
...
public:
  ...
  void dbscanSeg(pcl::PointCloud<PointL>::Ptr input_sem_cloud) {
    ...
    for (int i = 0; i < selected_points_->points.size(); i++) {
      ...
      for (int j = 0; j < region_max_; j++) {
        float d2 =
            selected_points_->points[i].x * selected_points_->points[i].x +
        ...
        for (std::vector<pcl::PointIndices>::const_iterator it =
          ...
          for (int j = last_clusters_begin; j < last_clusters_end; j++) {
            ...
            if (clusters_[j]->points.size() >= 1) {
              if (kdtree.nearestKSearch(*clusters_[j],
                                        clusters_[j]->points.size() - 1, K,
                                        k_indices, k_sqr_distances) > 0) {
                if (k_sqr_distances[0] < k_merging_threshold_) {
                  *cluster += *clusters_[j];
                  clusters_.erase(clusters_.begin() + j);
                  last_clusters_end--;
                  // std::cerr << "k-merging: clusters_ " << j << " is merged"
                  // << std::endl;
                }
              }
            }
          ...
          for (int k = j + 1; k < clusters_.size(); k++) {
            ...
            if (std::max(std::min((double)j_max[0], (double)k_max[0]) -
              ...
              *clusters_[j] += *clusters_[k];
              ...
              // std::cerr << "z-merging: clusters_ " << k << " is merged into "
              // << j << std::endl;
            }
          }
        }
  ...
  void computeCentroidAndCov(
      pcl::PointCloud<PointType>::Ptr cluster_centroid,
    ...
    for (int i = 0; i < cluster_size; ++i) {
      ...
      // compute cov 3*3
      Eigen::Matrix3d cov =
          neighbors * neighbors.transpose() / clusters_[i]->size();
  ...
  void computeCov(
      pcl::PointCloud<PointType>::Ptr original_pt_cloud,
    ...
    for (size_t i = 0; i < index.size(); ++i) {
      int cur_idx = index[i];
      neighbors.col(i) =
          original_pt_cloud->at(cur_idx).getVector3fMap().cast<double>();
    }
  ...
  void createHashTable() {
    ...
    for (size_t item = 0; item < totalSize; ++item) {
      ...
      pitchIndex =
          static_cast<int>(std::round((cur.y() - minPitch) / params_.deltaP));
  ...
  bool DCVC(std::vector<int> &label_info) {
    ...
    if (totalSize <= 0) {
      ROS_ERROR(
          "points in the cloud not enough to complete the DCVC algorithm");
      return false;
    }
   ...
   * @return void
   */
  void searchKNN(int &polar_index, int &pitch_index, int &azimuth_index,
  ...
  bool
  fpfh_feature_extraction(pcl::PointCloud<PointL>::Ptr input_src_sem_cloud,
                          pcl::PointCloud<PointL>::Ptr input_tgt_sem_cloud) {
    // pointCloudL -> pointCloud
    ...
    pcl::PointCloud<pcl::Normal>::Ptr src_normals_raw(
        new pcl::PointCloud<pcl::Normal>);
    pcl::PointCloud<pcl::Normal>::Ptr tgt_normals_raw(
        new pcl::PointCloud<pcl::Normal>);
    ...
    src_descriptors_ =
        fpfh.computeFPFHFeatures(src_cloud_, *src_normals_raw, 1, 2.5);
    tgt_descriptors_ =
        fpfh.computeFPFHFeatures(tgt_cloud_, *tgt_normals_raw, 1, 2.5);

    teaser::Matcher matcher;
    corr_ = matcher.calculateCorrespondences(
        src_cloud_, tgt_cloud_, *src_descriptors_, *tgt_descriptors_, true,
        true, true, 0.95);
    ...
    for (size_t i = 0; i < corr_.size(); ++i) {
      ...
      src_matched_.col(i) << src_cloud_[src_idx].x, src_cloud_[src_idx].y,
          src_cloud_[src_idx].z;
      tgt_matched_.col(i) << tgt_cloud_[dst_idx].x, tgt_cloud_[dst_idx].y,
          tgt_cloud_[dst_idx].z;
    }
    ...
    for (size_t i = 0; i < corr_.size(); ++i) {
      ...
      int size_src =
          src_tree.radiusSearch(temp_src, 1, src_neighbor_index, src_dist);
      computeCov(srcFeat, src_neighbor_index, covariances_src_);
      int size_tgt =
          tgt_tree.radiusSearch(temp_tgt, 1, tgt_neighbor_index, tgt_dist);
