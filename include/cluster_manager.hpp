...
public:
    ...
    bool selectSemanticPoints(pcl::PointCloud<PointL>::Ptr input_src_sem_cloud, pcl::PointCloud<PointL>::Ptr input_tgt_sem_cloud) {
                ...
                for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++) {
                    ...
                    for(int j = last_clusters_begin; j < last_clusters_end; j++) {
                        ...
                        if(clusters_[j]->points.size() >= 1) {
                            if(kdtree.nearestKSearch(*clusters_[j], clusters_[j]->points.size()-1, K, k_indices, k_sqr_distances) > 0) {
    ...
    void setParams(int semantic_class, double cluster_distance_threshold, int minNum, int maxNum, clusterManager::ClusterParams& params) {
    ...
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> getSrcCovMat() {
        return covariances_src_;
    }

    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> getTgtCovMat() {
...
private:
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<pcl::PointXYZ>::Ptr > > clusters_;
    ...
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> covariances_src_;
    std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> covariances_tgt_;
