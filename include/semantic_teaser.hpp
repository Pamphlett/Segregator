  ...
  struct Params {
    ...
    Params() {
      ...
      teaser_params.rotation_estimation_algorithm =
          TEASER::ROTATION_ESTIMATION_ALGORITHM::GNC_TLS;
      ...
      teaser_params.inlier_selection_mode =
          TEASER::INLIER_SELECTION_MODE::PMC_EXACT;
    }
  };
...
public:
  semanticTeaser(semanticTeaser::Params params)
      : params(params), solver(params.teaser_params) {
  ...
  void solve_for_bg_points(const pcl::PointCloud<pcl::PointXYZ> &src,
   ...
   * \param[in] src_cloud_vec vector containing source clouds of different
   * semantics \param[in] tgt_cloud_vec vector containing target clouds of
   * different semantics
   */
  void solve_for_multiclass(
      const std::vector<pcl::PointCloud<pcl::PointXYZ>> &src_cloud_vec,
      const std::vector<pcl::PointCloud<pcl::PointXYZ>> &tgt_cloud_vec);

  /** \brief solve cloud-wise transformation using multiple semantic clusters
   * with cov mat outlier pruning \param[in] src_cloud_vec vector containing
   * source clouds of different semantics \param[in] tgt_cloud_vec vector
   * containing target clouds of different semantics \param[in] src_covariances
   * vector of eigen matrix containing cov mats of each point in src cloud
   * \param[in] tgt_covariances vector of eigen matrix containing cov mats of
   * each point in tgt cloud
   */
  void solve_for_multiclass_with_cov(
      const std::vector<pcl::PointCloud<pcl::PointXYZ>> &src_cloud_vec,
      const std::vector<pcl::PointCloud<pcl::PointXYZ>> &tgt_cloud_vec,
  ...
  void setInliers(const Eigen::Matrix<double, 3, Eigen::Dynamic> &raw,
  ...
  inline Eigen::Matrix<double, 3, Eigen::Dynamic> getMaxCliqueSrcTIMs() {
    return solver.getMaxCliqueDstTIMs();
  }
  ...
  /**
   * Return a boolean Eigen row vector indicating whether specific
   * measurements are inliers according to the rotation solver.
   *
   * @return a 1-by-(size of TIMs used in rotation estimation) boolean Eigen
   * matrix. It is equivalent to a binary mask on the TIMs used in rotation
  ...
  inline Eigen::Matrix<bool, 1, Eigen::Dynamic> getRotationInliersMask() {
    return solver.getRotationInliersMask();
  }
};
